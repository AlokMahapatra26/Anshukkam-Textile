...
async function createBucket() {
    ...
    // Create factory-photos bucket
    const { data, error } = await supabase.storage.createBucket('factory-photos', {
        public: true,
    ...
    if (error) {
        if (error.message.includes('already exists')) {
            console.log('✅ Bucket "factory-photos" already exists');
        } else {
            console.error('Error creating bucket:', error.message);
        }
    } else {
        console.log('✅ Bucket "factory-photos" created successfully!');
    }
